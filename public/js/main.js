...
socket.on('join_room_response',function(payload){
	...
	var newHTML = '<p><font color="Teal"><b>'+payload.username+'</b> just entered the room</font></p>';
	var newNode = $(newHTML);
	newNode.hide();
...
/* What to do when the server say that someone has left a room */
socket.on('player_disconnected',function(payload){
	...
	var newHTML = '<p><font color="SkyBlue">'+payload.username+' just left the room</font></p>';
	var newNode = $(newHTML);
	newNode.hide();
	$('#messages').prepend(newNode);
	newNode.slideDown(1000);
...
/* handle response after ending an invite message to server */	
...
/* handle response after ending an uninvite message to server */	
...
socket.on('play_token_response',function(payload){
	...
	console.log('*** Client Log Message: \'play_tokem_response\' \n\tpayload: '+JSON.stringify(payload));
	/* Check for a good play_token_repsonse  */
	if(payload.result == 'fail'){
		console.log(payload.message);
		alert(payload.message);
		return;
